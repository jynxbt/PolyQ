...
import { spawnDetached, killByPattern, isProcessRunning, killPort } from '../process'
import { waitUntilReady, httpHealthCheck } from '../health'
...
export function createValidatorStage(options: ValidatorStageOptions): Stage {
  ...
  return {
    ...
    async start() {
      ...
      await waitUntilReady(
        async () => {
          const { portCheck } = await import('../health')
